...
using RoutesMap = std::unordered_map<string, std::unique_ptr<listen_routes>>;
    ...
    public:
        ...
        auto getMainProcess(const shared_ptr<RoutesMap> &_routes, const neo::LISTEN_TYPE _listen_type = neo::WHILE) {
                ...
                request_t = make_shared<RequestIO>(_routes, file_descriptor, epoll_fd, _listen_type,
                    [this]() {
                        kill();
                    });
                const int clock_speed = _listen_type == neo::WHILE ? CLOCK_SPEED : -1;
                ...
                try {
                    ...
                    std::this_thread::sleep_for(std::chrono::milliseconds(SLEEP_AFTER_KILL_IN_MS));
                    close(epoll_fd);
                    close(file_descriptor);
                    request_t.reset();
                    terminal("epoll::finished");
        }
        ...
        void kill() {
            listen_status_ = enums::neo::eStatus::STOP;
            terminal("router_epoll::kill");
        }
    };
}
