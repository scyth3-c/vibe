...
class Request {
    ...
    vector<vibe::http::UploadedFile> _files{}; // multipart/form-data uploads
...
public:
    ...
    [[nodiscard]] inline const vector<vibe::http::UploadedFile>& files() const noexcept { return _files; }

    // First uploaded file for a given form field, nullptr when absent.
    [[nodiscard]] const vibe::http::UploadedFile* file(std::string_view field) const noexcept {
