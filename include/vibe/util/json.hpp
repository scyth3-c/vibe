...
//
// vibe::Json — DOM (tree) JSON value with strict parsing (RFC 8259) and
...
//
//   const auto body = vibe::Json::parse(web.body.raw()); // parse
//   if (!body) return web.json(R"({"error":"invalid json"})", 400);
//   const vibe::Json* id = body->at("id");            // nullptr when absent
//
...
// safely on top of vibe::Json: proper escaping, native number/bool types,
// real nesting and no out-of-bounds reads (an odd trailing key now yields a
// null value instead of reading past the list). Prefer vibe::Json in new code.
//
...
public:
    [[maybe_unused]] JsonStringBuilder(std::initializer_list<vibe::Json> items) {
        vibe::Json::object_t root;
        auto it = items.begin();
        while (it != items.end()) {
            std::string key(it->as_string());
            vibe::Json value = nullptr;
            if (++it != items.end()) {
                value = *it;
                ++it;
            }
