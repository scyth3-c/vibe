...
// access through the global vermell::environment object:
//
//   vermell::environment.get("TOKEN")             // .env / set(), else OS env, else ""
//   vermell::environment.get("TOKEN", "fallback")
//   vermell::environment.get_as<int>("PORT", 8080) // typed conversion
//   vermell::environment["TOKEN"]
//   vermell::environment.set("request_count", "42")  // runtime session value
//
...
#ifndef VERMELL_ENVIRONMENT_H
#define VERMELL_ENVIRONMENT_H
...
namespace vermell {
    ...
    // The single instance behind vermell::environment (initialized on first use).
    Environment& environment_instance();

    // Node.js-style global: vermell::environment.get(...)
    inline Environment& environment = environment_instance();

} // namespace vermell

#endif // VERMELL_ENVIRONMENT_H
